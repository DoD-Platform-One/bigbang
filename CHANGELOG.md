...
## [2.10.7]
### Changed
- Add logic to create elasticsearchKibana BB MRs with fluentbit label for CI testing.

## [2.10.6]
### Changed
- Revert workflow rules applied to bigbang-bot scheduled job
...
## [2.10.4]
### Changed
- Adding SBOM syft requirement to bigbang-bot.yaml
...
## [2.9.22]
### Changed
- Updating pipelines/bigbang.yaml and library/templates.sh to correctly execute bb-docs for RC release tags
...
## [2.9.19]
### Changed
- incorporating the new AWS Terraform EKS Module into the pipeline

## [2.9.18]
### Changed
- select "branch" key under "git" when processing tests/dependencies.yaml
...
## [2.9.10]
### Added
- Fix helm uninstall in big bang down stage. Implement error handling to pass job when timeout error is received from helm uninstall cmd.

## [2.9.9]
### Added
- Fix helm uninstall in big bang down stage. Allows all PVs to be cleaned up.

## [2.9.8]
### Added
- updated image to 2.9.4 which contains the OSCAL aggregator

## [2.9.7]
### Changed
- removing image call to python image for bigbang-bot pipeline 

## [2.9.6]
### Changed
- Fix bb-docs-compiler job git clone repo path

## [2.9.5]
### Changed
- RKE2 and EKS job rules

## [2.9.4]
### Added
- added OSCAL aggregator to bb-ci dockerfile

## [2.9.3]
### Changed
- Airgap and other job rules

## [2.9.2]
### Added
- Automation for Airgap Testing with Zarf of Big Bang Releases.

## [2.9.1]
### Changed
- changed OSCAL validation to fail pipelines

## [2.9.0]
### Changed
- Setup .pre stage for bigbang pipeline to check for opensource branch name PR-{number} and block pipeline behind a manual gate
- Setup .pre stage for bigbang packages pipeline to check for opensource branch name PR-{number} and block pipeline behind a manual gate
- Setup .pre stage for sandbox packages pipeline to check for opensource branch name PR-{number} and block pipeline behind a manual gate
- Setup .pre stage for third party packages pipeline to check for opensource branch name PR-{number} and block pipeline behind a manual gate

## [2.8.7]
### Fixed
-  hotfix x2 to pipeline schedule

## [2.8.6]
### Fixed
-  pre vars job trigger in bigbang.yaml
...
## [2.7.7]
### Changed
- Updated Zarf to v0.27.0

## [2.7.6]
### Added
- Correct placement for logic regarding integration stage schema.json replacements

## [2.7.5]
### Added
- Workaround for integration stage schema.json errors

## [2.7.4]
### Added
- fixing kubectl get virtualservices script

## [2.7.3]
### Added
- Including Zarf executable.

## [2.7.2]
### Changed
- Remove Micah/Rob and add Chris

## [2.7.1]
### Added
- Disable jira job in bb bot pipeline.
...
## [2.6.18]
### Changed
- Bigbang Release now kicks off the BB Docs Compiler Pipeline
## [2.6.18]
### Changed
...
## [2.6.17]
### Changed
- Add nightly scheduled job to bb pipeline to test helmRepo/oci clean install of core packages using tests/oci-values.yaml

## [2.6.16]
### Fixed
- Fix edge case where images list contains no images (library charts, etc)

## [2.6.15]
### Changed
- Fix package variable in bigbang_package_images, add test for bigbang_package_images

## [2.6.14]
### Changed
- Modified yq for packaging repos to support `[]` present in values

## [2.6.13]
### Changed
- OCI -> HelmRepo change for yq things

## [2.6.12]
### Changed
- Update RKE2 nightly version to 1.26.3
...
## [2.6.9]
### Fixed
- Fixed dependency mapping to handle string things
...
## [2.6.6]
### Changed
- Add basic test script and Makefile

## [2.6.5]
### Changed
- Refactor deploy_bigbang.sh to deploy all of core and use helper functions

## [2.6.4]
### Added
- Increase artifactFailed counter in wait_for_hr() function to prevent pipeline failures when pulling OCI artifacts. Again to 50

## [2.6.3]
### Added
- Added dump of helmrepos at end of pipeline

## [2.6.2]
### Added
- Increase artifactFailed counter in wait_for_hr() function to prevent pipeline failures when pulling OCI artifacts. Again.

## [2.6.1]
### Added
- Increase artifactFailed counter in wait_for_hr() function to prevent pipeline failures when pulling OCI artifacts

## [2.6.0]
### Added
- Package stage and OCI creation job to 3rd party and sandbox pipelines

## [2.5.34]
### Fixed
- Hotfix for label dependency lookup

## [2.5.33]
### Fixed
- Hotfix for allowed failure on tagging
...
## [2.5.28]
### Fixed
- Modified auto-BB MR logic to exit if no MR found

## [2.5.27]
### Fixed
- Modified image lists to pull from all nodes rather than just server

## [2.5.26]
### Changed
- Modified logic surrounding BB MR creation to definitively grab the package MR
- Added assignee to the BB MR
...
## [2.5.15]
### Changed
- Mattermost standardization

## [2.5.14]
### Changed
- increased retries on artifact failed HR error

## [2.5.13]
### Changed
- updated gitlab-triage image tag

## [2.5.12]
### Changed
- rename nexus to nexusRepositoryManager

## [2.5.11]
### Changed
- Modify package image parsing to follow redirects properly

## [2.5.10]
### Changed
- Fixed parsing of git repo url to strip off `platform-one/`

## [2.5.9]
### Changed
- Fixed one more curl spot

## [2.5.8]
### Changed
- Changed curls to follow redirects
- Updated all URLs to new top level group

## [2.5.7]
### Added
- Added blank initial.yaml pipeline

## [2.5.6]
### Changed
- Moved all images to new location in registry1

## [2.5.5]
### Fixed
- Ignore exit code of `find` for test values

## [2.5.4]
### Changed
- Modified kyverno policy script to handle edge cases

## [2.5.3]
### Added
- Kyverno policy test script
- Kyverno cli to bb-ci image

### Updated
- Helm version to `3.10.2` in bb-ci image
- Removed `conftest` from bb-ci image
...
## [2.4.93]
### Updated
- Removed sandbox check bigbang_package_images
...
## [2.4.91]
### Added
- Add Neuvector to list of packages
...
## [2.4.89]
### Changed
- sandbox pipelines integration stage config

## [2.4.88]
### Changed
- Spam Rob with emails

## [2.4.87]
### Changed
- Added OCI Creation for bigbang the product

## [2.4.86]
### Changed
- Fixed CI for airgap AMI Builder

## [2.4.85]
### Added
- Added CI for airgap AMI Builder

## [2.4.84]
### Changed
- Modified handling of blank lines by MR creation

## [2.4.83]
### Changed
- Removed directory path from checksum file

## [2.4.82]
### Added
- Implementing OCI storage for helm.yaml and bigbang-package.yaml

## [2.4.81]
### Changed
- Implementing Sha256 hash to asset links

## [2.4.80]
### Updated
- Updated bigbang-docs pipeline to add back tests and removed some debug commands
...
## [2.4.77]
### Added
- k3d can now have multiple nodes enabled through a CI variable `MULTI_NODE`

## [2.4.76]
### Changed
- Relocated Gitlab-Triage logic to pipeline-templates

## [2.4.75]
### Changed
- Modified bigbang docs pipeline for scheduled pipelines
...
## [2.4.72]
### Changed
- Fixed issue with MR labels being processed twice (and duplicate log entries showing up)

## [2.4.71]
### Changed
- Moved gitlab-triage to new runners

## [2.4.70]
### Changed
- Moved jobs to new runner tags
- Updated rke2 deployment to new prod environment

## [2.4.69]
### Changed
- Update pipelines/bigbang-docs.yaml scheduled job's git clone command
...
- Add dependencies into bb-ci, bump version to 2.4.67

## [2.4.66]
### Added
- new pipeline for gitlab-triage
...
## [2.4.62]
### Changed
- Updated helm-docs deps

## [2.4.61]
### Fixed
- Added metrics server yaml key remapping for values/BB MR creation

## [2.4.60]
### Fixed
- Fixed `bigbang_package_images` to not have weird bash redirection, uses simple for loop and leverages yq a lot better (imo)
- Change exit code from `321` to `99`, as bash exit codes are only `1-255`

## [2.4.59]
### Added
- Added `kubectl patch psp global-unrestricted-psp -p '{"spec":{"allowedUnsafeSysctls":["*"]}}'` to rke2 pipelines to allow gatekeeper and kyverno helm tests to work correctly.
...
- Moved verbose service `curl` to debug
- Grammer, color codes, and spelling fixes
- Command output supression (`1>/dev/null`)

## [2.4.56]
### Changed
- Added condition to skip BB merge requests for non-BB packages

## [2.4.55]
### Changed
- Renamed package_oscal_validate to oscal_validate

## [2.4.54]
### Added
- Added OSCAL validation function to Big Bang pipeline

## [2.4.53]
### Fixed
- Integration stage failed during virtualservices test due to expired ingress cert
### Added
- Added `git checkout` command to the `clone_bigbang_and_merge_templates` function to pull in updated certs

## [2.4.52]
### Fixed
- Fixed some more bugs with the integration stage and vault/metricsServer conditionals
...
## [2.4.49]
### Fixed
- main/master pipeline for BB running metrics server
### Added
- Added HR waits for metrics-server

## [2.4.48]
### Changed
- Reverted use of `\\p` regex in OSCAL schema

## [2.4.47]
### Changed
- Updated OSCAL schema version from `1.0.2` to `1.0.4`

## [2.4.46]
### Fixed
- Fixed scheduled pipelines running incorrect cluster build

## [2.4.45]
### Fixed
- Fixed label variable placement in rke2 deployment
...
## [2.4.42]
### Added
- Added bigbang-docs.yaml pipeline

## [2.4.41]
### Changed
- Changed rke2 bigbang test stage to no longer retry on runner failure
