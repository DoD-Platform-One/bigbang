...
if [[ "${PIPELINE_TYPE}" == "BB" ]]; then
  ...
  # Enable kyverno reporter
  if [[ "${CI_DEPLOY_LABELS[*]}" =~ "kyvernoreporter" ]] || [[ "${CI_COMMIT_BRANCH}" == "${CI_DEFAULT_BRANCH}" ]] || [[ ! -z "$CI_COMMIT_TAG" ]] || [[ ${CI_DEPLOY_LABELS[*]} =~ "all-packages" ]]; then
      echo "Enabling kyverno reporter"
      yq e ".kyvernoreporter.enabled = "true"" $CI_VALUES_FILE > tmpfile && mv tmpfile $CI_VALUES_FILE
  fi
