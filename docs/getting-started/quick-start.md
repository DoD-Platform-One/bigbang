...
Spin up an Ubuntu VM with 8 CPUs and 32 GB of RAM. Ensure you can SSH to it. The cloud provider does not matter; it can be on your local system if it has sufficient resources.
...
Default credentials for Big Bang packages are documented in [Default Credentials](../configuration/default-credentials.md).

### Tinker With It

You can use the Quick Start script to update your Big Bang Quick Start deployment with your own modifications. Here's an example of post-deployment customization of Big Bang. After looking at [`chart/values.yaml`](../../chart/values.yaml), you can see that this will enable the Argo CD add-on, which is not enabled by default.
...
Big Bang by itself serves as a jumping-off point, but many users will want to implement their own mission-specific applications in the cluster. Big Bang provides `packages` and `wrapper` configuration for integrating those applications with capabilities such as Istio, monitoring, and network policy management. See [Extra Package Deployment](../installation/environments/extra-package-deployment.md).
...
Wrappers also allow you to abstract monitoring, Secrets, NetworkPolicies, and ConfigMaps. See [Extra Package Deployment](../installation/environments/extra-package-deployment.md) for additional configuration.
...
* HTTPS Certificate and hostname configuration Prerequisites: Are satisfied by leveraging default hostname values and the demo HTTPS wildcard certificate that's uploaded to the Big Bang repo, which is valid for *.dev.bigbang.mil, *.admin.dev.bigbang.mil, and a few others. The demo HTTPS wildcard certificate is signed by the Lets Encrypt Free, a Certificate Authority trusted on the public internet, so demo sites like grafana.dev.bigbang.mil will show a trusted HTTPS certificate.
...
After completing step 5, use `nslookup google.com 8.8.8.8` to test external DNS resolution. If it fails, use the steps below to change the upstream DNS server to your network's DNS server. This change does not persist after the host restarts, so you must reapply it after restarting or shutting down the server.
